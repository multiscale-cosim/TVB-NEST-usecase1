...
class ZerlautNESTModel:
    def __init__(self, cosimulation, param_topology):
         # Number of region to simulate
        self.__nb_region = len(cosimulation['id_region_nest'])

        # Number of neurons by region
        self.__nb_neuron_region = param_topology['nb_neuron_by_region']
        # Percentage of inhibitory neurons
        self.__percentage_inhibitory = param_topology['percentage_inhibitory']
        # Type of neuron
        self.__neuron_type = param_topology['neuron_type']
        # Parameter of excitatory neuron (different to default value)
        self.__params_ex = param_topology['param_neuron_excitatory']
        self.__neuron_type_ex = 'excitatory'
        # Parameter of excitatory neuron (different to default value)
        self.__params_in = param_topology['param_neuron_inhibitory']
        self.__neuron_type_in = 'inhibitory'
        # Mean of external input
        self.__mean_I_ext = param_topology['mean_I_ext']
        # Sigma of external input
        self.__sigma_I_ext = param_topology['sigma_I_ext']
        # Sigma of initial voltage
        self.__sigma_init_V0 = param_topology['sigma_V_0']
        # mean of initial voltage
        self.__mean_init_w0 = param_topology['mean_w_0']
    ...
    def __network_initialisation_neurons(self, results_path, param_topology, cosimulation, nest, return_gids=False):
        """
        ...
        nest.CopyModel(self.__neuron_type, self.__neuron_type_ex)
        # nest.SetDefaults(neuron_type_ex, params_ex)
        nest.SetDefaults(self.__neuron_type_ex, self.__params_ex)
        # nest.CopyModel(neuron_type, neuron_type_in)
        nest.CopyModel(self.__neuron_type, self.__neuron_type_in)
        # nest.SetDefaults(neuron_type_in, params_in)
        nest.SetDefaults(self.__neuron_type_in, self.__params_in)

        # Create all Units
        nb_ex = int(self.__nb_neuron_region * (1 - self.__percentage_inhibitory))  # number of excitatory neurons
        nb_in = int(self.__nb_neuron_region * self.__percentage_inhibitory)  # number of inhibitory neurons
        ...
        for j in range(0, self.__nb_region):
            # Excitatory region
            reg_ex = nest.Create(self.__neuron_type_ex, nb_ex)
            ...
            if self.__sigma_I_ext > 0:
                reg_ex.set(I_e=nest.random.normal(mean=self.__mean_I_ext, std=self.__sigma_I_ext))
            else:
                reg_ex.set(I_e=self.__mean_I_ext)
            if self.__sigma_init_V0 > 0:
                # reg_ex.set(V_m=nest.random.normal(mean=nest.GetDefaults(neuron_type_ex)['E_L'], std=sigma_init_V0))
                reg_ex.set(V_m=nest.random.normal(mean=nest.GetDefaults(self.__neuron_type_ex)['E_L'], std=self.__sigma_init_V0))
            else:
                reg_ex.set(V_m=nest.GetDefaults(self.__neuron_type_ex)['E_L'])
            if self.__mean_init_w0 > 0:
                reg_ex.set(w=nest.random.normal(mean=self.__mean_init_w0, std=self.__mean_init_w0))
            else:
                reg_ex.set(w=self.__mean_init_w0)

            # Inhibitory region
            reg_in = nest.Create(self.__neuron_type_in, nb_in)
            ...
            if self.__sigma_I_ext > 0:
                reg_in.set(I_e=nest.random.normal(mean=self.__mean_I_ext, std=self.__sigma_I_ext))
            else:
                reg_in.set(I_e=self.__mean_I_ext)
            if self.__sigma_init_V0 > 0:
                reg_in.set(V_m=nest.random.normal(mean=nest.GetDefaults(self.__neuron_type_in)['E_L'], std=self.__sigma_init_V0))
            else:
                reg_in.set(V_m=nest.GetDefaults(self.__neuron_type_in)['E_L'])
    ...
    def __init_connection(self, dic_layer, param_topology, param_connection, nest):
        """
    ...
    def __create_homogenous_connection(self, dic_layer, param_topology, param_connection, nest, save=False, init=None):
        """
    ...
    def __create_heterogeneous_connection(self, dic_layer, param_topology, param_connection, nest, save=False, init=None,
                                        cosimulation=None):
        """
        ...
        if init is None:
            (conn_params_ex_inside, conn_params_in_inside,
            list_layer_ex, list_layer_in,
            weights, delays) = self.__init_connection(dic_layer, param_topology, param_connection)
        else:
            (conn_params_ex_inside, conn_params_in_inside,
            list_layer_ex, list_layer_in,
            weights, delays) = init
    ...
    def __network_connection(self, dic_layer, param_topology, param_connection, cosimulation=None):
        """
        Create the connection between all the neurons
        :param dic_layer: Dictionary with all the layer
        :param param_topology: Parameter for the topology
        ...
        :return: nothing
        """
        init = self.__init_connection(dic_layer, param_topology, param_connection)
        self.__create_homogenous_connection(dic_layer, param_topology, param_connection, save=False, init=init)
        self.__create_heterogeneous_connection(dic_layer, param_topology, param_connection, save=False, init=init,
                                        cosimulation=cosimulation)


    def __network_device(self, results_path, dic_layer, min_time, time_simulation, param_background, param_connection,
                         mpi=False, cosimulation=None):
        """
    ...
    def __simulate(self, results_path, begin, end,
        ...
        if nest.Rank() == 0:
            tic = time.time()
        self.__network_initialisation(results_path, param_nest)
        dic_layer = self.__network_initialisation_neurons(results_path, param_topology)
        if nest.Rank() == 0:
            toc = time.time() - tic
        ...
        if nest.Rank() == 0:
            tic = time.time()
        self.__network_connection(dic_layer, param_topology, param_connection)
        spike_detector, spike_generator = self.__network_device(results_path, dic_layer, begin, end, param_background,
                                                        param_connection)
        if nest.Rank() == 0:
            toc = time.time() - tic
    ...
    def __config_mpi_record(self, results_path, begin, end,
                        param_nest, param_topology, param_connection, param_background,
                        cosimulation=None):
        """
        ...
        # Initialisation of the network
        self.__network_initialisation(results_path, param_nest)
        dic_layer = self.__network_initialisation_neurons(results_path, param_topology, cosimulation=cosimulation)

        # Connection and Device
        self.__network_connection(dic_layer, param_topology, param_connection, cosimulation=cosimulation)
        spike_detector, spike_generator = self.__network_device(results_path, dic_layer, begin, end, param_background,
    ...
    def __simulate_mpi_co_simulation(self, time_synch, end, logger):
        """
